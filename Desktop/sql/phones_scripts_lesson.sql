...
--выведет все потому что все ко всему  (x<>1) and (x<>2)
select * from phones_samsung
where price <> any (select price from phones_apple where price < 1000);



-- должны совпадать столбцы их количество и тип данных 
select ram, price, model from phones_apple where price < 1000
union all 
select ram, price, model from phones_samsung where price < 1000;


select ram, count(model) from phones_apple
group by ram;
...
--select count(*) from phones_apple where price<1000;
